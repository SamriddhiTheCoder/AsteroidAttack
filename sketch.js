...
function setup(){
  ...
  //earth
  earth = createSprite(displayWidth/2, 100);
...
function draw(){
  ...
  if(gameState === START){
    ...
    push();
    imageMode (CENTER);
    image(title, width/2, 50, 400, 400);
    image(start, width/2, height/2, 400, 400);
    textSize(30);
    fill ("black");
    text("Press 'P' to play", width/2-80, height/2+120);
    pop ();
    if(keyDown("p")){
      gameState = PLAY;
    }
  ...
  if(keyDown("right")){
    ...
    for(var i = 0;i < group.length; i++){
      ...
      if(count !=0){
        textSize(20);
        text(count + "Lives left before explosion", width-200,50);
      }
      pop()
    }
